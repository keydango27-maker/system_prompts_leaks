<!-- 简体中文机器翻译；仅供检索、阅读和研究。 -->
<!-- source-file: Anthropic/Claude Code/bundled-skills/update-config.md -->
<!-- source-sha256: bdf7a9559f01d3695bfcee7cb95f7884879e028119ba75eb8aa13ce5ac52d134 -->
...
如果用户希望自动发生某些事件以响应事件，则需要在 settings.json 中配置 **hook**。内存/首选项无法触发自动操作。

**这些需要钩子：**
- “压缩之前，问我要保留什么” → PreCompact 钩子
- “写入文件后，运行 prettier” → PostToolUse 与 Write|Edit 匹配器挂钩
- “当我运行 bash 命令时，记录它们”→带有 Bash 匹配器的 PreToolUse 挂钩
...
|文件 |范围 | git | git用于 |
|------|--------|-----|---------|
| `~/.claude/settings.json` |全球|不适用 |所有项目的个人偏好|
| `.claude/settings.json` |项目|提交 |团队范围的挂钩、权限、插件 |
| `.claude/settings.local.json` |项目|吉蒂诺尔 |此项目的个人覆盖|

设置加载顺序为：用户 → 项目 → 本地（稍后覆盖之前）。
...
```插件语法：`plugin-name@source`，其中源为 `claude-code-marketplace`、`claude-plugins-official` 或 `builtin`。

### 其他设置
- `language`：首选响应语言（例如，"japanese"）
- `cleanupPeriodDays`：自动清理前保留成绩单的天数（默认值：30；最少 1）
- `respectGitignore`：是否尊重.gitignore（默认值：true）
- `spinnerTipsEnabled`：在微调器中显示提示
- `spinnerVerbs`：自定义微调动词（`{ "mode": "append" | "replace", "verbs": [...] }`）
- `spinnerTipsOverride`：覆盖旋转器提示 (`{ "excludeDefault": true, "tips": ["Custom tip"] }`)
- `syntaxHighlightingDisabled`：禁用差异突出显示
...
|预紧凑 | "manual"/"auto" |压实前|
|后紧凑| "manual"/"auto" |压缩后（收到摘要）|
|用户提示提交 | - |当用户提交|
|会话开始 | - |会话开始时 |

**常用刀具匹配器：** `Bash`、`Write`、`Edit`、`Read`、`Glob`、`Grep`
...
- `decision` - "block" 用于 PostToolUse/Stop/UserPromptSubmit 挂钩（已弃用 PreToolUse，请改用 hookSpecificOutput.permissionDecision）
- `reason` - 决定说明
- `hookSpecificOutput` - 特定于事件的输出（必须包括 `hookEventName`）：
...
2. **构建此项目的命令 - 不要假设。** 钩子接收JSON在标准输入上。构建一个命令：
   - 安全地提取任何所需的有效负载 - 使用`jq -r`进入带引号的变量或`{ read -r f; ... "$f"; }`, 不加引号`| xargs`（按空格分割）
   - 以该项目运行的方式调用底层工具（npx/bunx/yarn/pnpm？Makefile 目标？全局安装？）
   - 跳过工具无法处理的输入（格式化程序通常有`--ignore-unknown`;如果没有，则通过扩展来保护）
   - 目前保持原始状态 - 不`|| true`，没有 stderr 抑制。管道测试通过后，您将包裹它。

3. **通过管道测试原始命令。** 合成钩子将接收的标准输入有效负载并直接通过管道传输：
   -`Pre|PostToolUse`在`Write|Edit`: `echo '{"tool_name":"Edit","tool_input":{"file_path":"<a real file from this repo>"}}' | <cmd>`
   - `Pre|PostToolUse`在`Bash`: `echo '{"tool_name":"Bash","tool_input":{"command":"ls"}}' | <cmd>`
   - `Stop`/`UserPromptSubmit`/`SessionStart`：大多数命令不读取标准输入，所以`echo '{}' | <cmd>`就够了

   检查退出代码和副作用（文件实际格式化，测试实际运行）。如果你失败了get一个真正的错误 - 修复（错误的包管理器？工具未安装？jq 路径错误？）并重新测试。工作完成后，用`2>/dev/null || true`（除非用户想要阻止检查）。

4. **写下JSON.** 合并到目标文件中（上面“钩子结构”部分中的架构形状）。如果这会创建`.claude/settings.local.json`第一次，将其添加到 .gitignore — Write 工具不会自动 gitignore 它。

5. **一次性验证语法+架构：**`jq -e '.hooks.<event>[] | select(.matcher == "<matcher>") | .hooks[] | select(.type == "command") | .command' <target-file>`Exit 0 + 打印你的命令=正确。退出 4 = 匹配器不匹配。出口 5 = 格式错误JSON或错误的嵌套。一个损坏的设置。json默默地禁用该文件中的所有设置 - 也修复任何预先存在的畸形。

6. **证明钩子点火** - 仅适用于`Pre|PostToolUse`在匹配器上你可以依次触发（`Write|Edit`通过编辑，`Bash`通过Bash). `Stop`/`UserPromptSubmit`/`SessionStart`在本回合外开火——跳至步骤 7。

   对于**格式化程序**`PostToolUse`/`Write|Edit`：通过编辑引入可检测到的违规（两个连续的空行、错误的缩进、缺少分号 - 此格式化程序会纠正这些内容；不是尾随空白，编辑在写入之前将其删除），重新阅读，确认钩子**已修复**它。对于**其他**：暂时在设置中为命令添加前缀。json和`echo "$(date) hook fired" >> /tmp/claude-hook-check.txt; `，触发匹配工具（编辑为`Write|Edit`, 无害的`true`为了`Bash`），读取哨兵文件。

   **始终清理** - 恢复违规，去除哨兵前缀 - 无论证明通过还是失败。

   **如果证明失败但管道测试通过并且`jq -e`通过**：设置观察者未观察`.claude/`— 它只监视此会话启动时具有设置文件的目录。钩子写得正确。告诉用户打开`/hooks`一次（重新加载配置）或重新启动 - 你不能自己这样做；`/hooks`是一个用户 UI 菜单，打开它会结束本回合。

7. **切换。** 告诉用户挂钩已激活（或需要`/hooks`/按照观察者警告重新启动）。将他们指向`/hooks`以便稍后查看、编辑或禁用它。如果钩子出错或速度缓慢，UI 仅显示“Ran N hooks”——按设计，静默成功是不可见的。
...
2. **阅读**：`.claude/settings.json`（如果缺失则创建）
3. **合并**：添加到现有的钩子中，不替换
4. **结果**：```json
{
  "hooks": {
...
2. **验证 JSON 语法** - 无效的 JSON 无提示失败
3. **检查匹配器** - 与工具名称匹配吗？ （例如，"Bash"、"Write"、"Edit"）
4. **检查挂钩类型** - 是 "command"、"prompt" 还是 "agent"？
